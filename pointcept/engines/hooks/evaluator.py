...
class MatchEvaluator(HookBase):
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            batch_gt = output_dict['target']
            loss = output_dict['loss']

            tp_list = []
            total_list = []
            for idx in range(len(batch_sim)):
                gt_mask = batch_gt[idx] != -1

                pred = torch.argmax(batch_sim[idx][gt_mask], dim=-1)
                gt = batch_gt[idx][gt_mask]

                tp = (pred==gt).sum()
                total = torch.tensor(batch_sim[idx][gt_mask].shape[0], device=tp.device)

                tp_list.append(tp.view(-1,))
                total_list.append(total.view(-1,))
            
            tp_list = torch.cat(tp_list)
            total_list = torch.cat(total_list)

            if comm.get_world_size() > 1:
                dist.all_reduce(tp_list), dist.all_reduce(total_list)

            tp, total = (
                tp_list.cpu().numpy(),
                total_list.cpu().numpy(),
            )
            # Here there is no need to sync since sync happened in dist.all_reduce
            self.trainer.storage.put_scalar("val_tp", tp)
            self.trainer.storage.put_scalar("val_all", total)
            self.trainer.storage.put_scalar("val_loss", loss.item())
            info = "Test: [{iter}/{max_iter}] ".format(
                iter=i + 1, max_iter=len(self.trainer.val_loader)
            )
            ...
            self.trainer.logger.info(
                ...
                + "Loss {loss:.4f}, Acc: {acc:.2f}%".format(
                    iter=i + 1, max_iter=len(self.trainer.val_loader), loss=loss.item(),
                    acc=sum(tp) / (sum(total) + 1e-10) * 100
                )
            )
        loss_avg = self.trainer.storage.history("val_loss").avg
        tp = self.trainer.storage.history("val_tp").total
        total = self.trainer.storage.history("val_all").total
        all_acc = sum(tp) / (sum(total) + 1e-10)
        self.trainer.logger.info("Val result: Acc {:.4f}.".format(all_acc))
        current_epoch = self.trainer.epoch + 1
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/allAcc", all_acc, current_epoch)
